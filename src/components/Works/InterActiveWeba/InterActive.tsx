'use client'
import { useEffect, useState } from "react";
import "./style.scss"; // Import your styles
import SectionTitle from "@/components/Work/SectionTitle";

const InteractiveWeba = () => {
  const [activeIndex, setActiveIndex] = useState(null);
  const [containerActive, setContainerActive] = useState(false);

  useEffect(() => {
    const timer = setTimeout(() => setContainerActive(true), 200);
    return () => clearTimeout(timer);
  }, []);

  const handleClick = (index: any) => {
    if (activeIndex === index) return;
    setActiveIndex(index);
  };

  const handleClose = (e: any) => {
    e.stopPropagation();
    setActiveIndex(null);
  };

  const sections = [
    "Library",
    "Chat",
    "3D",
  ];

  const content = [

    // first
    <>
      <div className="text-xl">

        Java, Spring Boot project " Library system "
        <br />
        that allows you to manage library of books to clients to borrow, retrieval books.
        <br />
        with all control functionality of books and users.
        <br />
        <br />
        <h3 className="text-3xl">Features</h3>
        <br />
        <ul>
          <li>Users Control " Create, Read, Edit, Delete Users ".</li>
          <li>Books Control " Create, Read, Edit, Delete Books ".</li>
          <li>Time-Frame, Fees " time/fees calculation for latency "</li>

        </ul>
        <br />
        <p>Watch Video &darr;</p>
        <br />
        <div className="flex flex-row gap-4 justify-center items-center">
          <iframe
            src="https://www.linkedin.com/embed/feed/update/urn:li:ugcPost:7320287393176035328?compact=1"
            height="399"
            width="504"
            frameBorder="0"
            allowFullScreen
            title="Embedded post"
          ></iframe>

        </div>
        <br />
        <h3 className="text-3xl">Technology Stack</h3>
        <br />
        <ul>
          <li>Frontend:
            <br />
            "HTML5, CSS3, Javascript, Bootstrap".</li>
          <br />
          <li>Backend:
            <br />
            " Java, Spring-Boot, Maven ".</li>
          <br />
          <li>Database " MySQL ".</li>
        </ul>
      </div>
    </>,



    // Full-stack Fully Functionality
    // Audio/Video Chat Conferencing,
    // Technologies : Next JS 14 - Shadcn designs,
    // clerk Authentication, Tailwind CSS.


    // second
    <>
      <div className="text-xl">

        Next.js project " Audio/Video Conference/Chat "
        <br />
        Full Functionalities Chat Conference website,
        <br />
        that allow to Chat, Record, Schedule meeting, and monitor connection latency.
        <br /><br />
        <h3 className="text-3xl">Features</h3>
        <br />
        <ul>
          <li>Authentication " OAuth by Google/Github, Login, Logout, Change Password ".</li>
          <li>Connection Analytics " both side can monitor connection status and latency ".</li>
          <li>Meeting Dashboard " Scheduled Coming Meetings, Meetings History".</li>
          <li>Recorded Meetings " Save, Play, Delete Records ".</li>
          <li>Seamless Reaction " During the call can make reactions to other ".</li>
          <li>Responsive " Support all media screens size ".</li>
        </ul>
        <br />
        <p>Watch Video &darr;</p>
        <br />
        <div className="flex flex-row gap-4 justify-center items-center">

          <iframe
            src="https://www.linkedin.com/embed/feed/update/urn:li:ugcPost:7193859513685311489?compact=1"
            height="399"
            width="504"
            frameBorder="0"
            allowFullScreen
            title="Embedded post"
          ></iframe>

        </div>
        <br />
        <h3 className="text-3xl">Technology Stack</h3>
        <br />
        <ul>
          <li>Next.js & Clerk Auth</li>
        </ul>
      </div>
    </>,




    // third
    <>
      <div className="text-xl">

        Next.js project " 3D Design "
        <br />
        Responsive 3D Design , personal portfolio website.
        <br />
        <br />
        <h3 className="text-3xl">Features</h3>
        <br />
        <ul>
          <li>3D Implementation " Acceleration, Rendering, 3D Models ".</li>
          <li>3D Anomation " Animate 3D Models ".</li>
          <li>Responsive " via all screens size ".</li>
        </ul>
        <br />
        <p>Watch Video &darr;</p>
        <br />
        <div className="flex flex-col gap-4 justify-center items-center">


          <iframe
            src="https://www.linkedin.com/embed/feed/update/urn:li:ugcPost:7188155651024445441?compact=1"
            height="399"
            width="504"
            frameBorder="0"
            allowFullScreen
            title="Embedded post"
          ></iframe>



        </div>
        <br />
        <h3 className="text-3xl">Technology Stack</h3>
        <br />
        <ul>
          <li>React.js - Next.js </li>
          <br />
          <li>Three.js, Framer Motion, MatrialUI, Styled Components </li>

        </ul>
      </div>
    </>,
  ];


  return (
    <div className="overflow-x-hidden md:overflow-x-auto">
      <div className='sticky-h3 w-fit text-xs' id="hello1">

        <SectionTitle title="Web-2" progressBgClass="bg-primary"></SectionTitle>

      </div>
      <div className={`cont md:w-[100%] md:-ml-0 w-[170%] -ml-[35%] ${containerActive ? "" : "s--inactive"} ${activeIndex !== null ? "s--el-active" : ""}`}>
        <div className="cont__inner">
          {sections.map((section, index) => (
            <div
              key={index}
              className={`el ${activeIndex === index ? "s--active" : ""}`}
              onClick={() => handleClick(index)}
            >
              <div className="el__overflow">
                <div className="el__inner">
                  <div className="el__bg2"></div>
                  <div className="el__preview-cont">
                    <h2 className="el__heading bg-sky-950 w-full text-center">{section}</h2>
                  </div>
                  <div className="el__content overflow-y-scroll overflow-x-hidden mx-auto">
                    <div className="el__text px-[15%]">Details</div>
                    {/* <img src="data:image/jpeg;base64," alt="" />
                    <img src="data:image/jpeg;base64," alt="" />
                    <img src="data:image/jpeg;base64," alt="" />
                    <img src="data:image/jpeg;base64," alt="" /> */}
                    <div className="text-center mx-auto bg-sky-950 opacity-90 p-5">
                      <h4 className="text-3xl ">Overview</h4>
                      <br />
                      {content[index]}
                    </div>
                    <br />


                    {/* <img src={media[index]} alt="" /> */}


                    <div className="el__close-btn mr-[15%]" onClick={handleClose}></div>
                  </div>
                </div>
              </div>
              <div className="el__index">
                <div className="el__index-back2">{index + 4}</div>
                <div className="el__index-front">
                  <div className="el__index-overlay" data-index={index + 4}>{index + 4}</div>
                </div>
              </div>
            </div>
          ))}
        </div>
      </div>
    </div>
  );
};

export default InteractiveWeba;
