'use client'
import { useEffect, useState } from "react";
import "./style.scss"; // Import your styles
import SectionTitle from "@/components/Work/SectionTitle";

const InteractiveWeb = () => {
  const [activeIndex, setActiveIndex] = useState(null);
  const [containerActive, setContainerActive] = useState(false);

  useEffect(() => {
    const timer = setTimeout(() => setContainerActive(true), 200);
    return () => clearTimeout(timer);
  }, []);

  const handleClick = (index: any) => {
    if (activeIndex === index) return;
    setActiveIndex(index);
  };

  const handleClose = (e: any) => {
    e.stopPropagation();
    setActiveIndex(null);
  };

  const sections = [
    "Proposal",
    "E-Commerce",
    "Blogging",
    // "Section 4",
    // "Section 5",
  ];

  const content = [

    // first
    <>
      <div className="text-xl">

        MERN Stack project " Proposal system "
        <br />
        that allows you to create and control forms to clients to collect submissions and set response to every selection.
        <br />
        with all input types and immediate response in same page and generate PDF file with entire proposal.
        <br />
        <br />
        <h3 className="text-3xl">Features</h3>
        <br />
        <ul>
          <li>Authentication " Login, Logout, Change Password, User Profile ".</li>
          <li>Forms " Create, Control, Edit, Delete, Data Binding, Suspension ".</li>
          <li>Submissions " Collect Responses, Dynamic Response, PDF Dynamic Proposal "</li>
          <li>Emails " Send Emails, Dynamic Content, Confirmation ".</li>
        </ul>
        <br />
        <p>Watch Video &darr;</p>
        <br />
        <div className="flex flex-row gap-4 justify-center items-center">
          <iframe
            src="https://www.linkedin.com/embed/feed/update/urn:li:ugcPost:7320279267458580481?compact=1"
            height="399"
            width="504"
            frameBorder="0"
            allowFullScreen
            title="Embedded post"
          ></iframe>

        </div>
        <br />
        <h3 className="text-3xl">Technology Stack</h3>
        <br />
        <ul>
          <li>Frontend: React.js
            <br />
            "Typescript, TailwindCSS, Dnd-kit, axios, radix-ui, tanstack/react-query, react-cookie, react-hook-form, zod, react-router-dom, react-to-print".</li>
          <br />
          <li>Backend: Node.js
            <br />
            " Express.js, Typescript, cors, jsonwebtoken, mongoose, multer, nodemailer ".</li>
          <br />
          <li>Database " MongoDB ".</li>
        </ul>
      </div>
    </>,






    // second
    <>
      <div className="text-xl">

        MERN Stack project " E-Commerce "
        <br />
        Full Functionalities E-Commerce website with Admin Dashboard, Sales Analytics, and Seamless Payment Integration.
        <br />
        with Feedback system to give stars rate and comment, and integration with analytics.
        <br /><br />
        <h3 className="text-3xl">Features</h3>
        <br />
        <ul>
          <li>Authentication " OAuth by Google/Github, Login, Logout, Change Password ".</li>
          <li>Autherization " Admin/Admin Dashboard, User/User Profile ".</li>
          <li>Products view " Create, Edit, Delete, Homepage view depends on best seller, and single product page for full details ".</li>
          <li>Admin Dashboard " Sales Analytics, Users Control, All Orders with its status ".</li>
          <li>Payment Methods " Availability to checkout and monitor the order status if pending or deliverd or faild ".</li>
          <li>Search & Filteration " Search by: Price range, Brand, Category ".</li>
          <li>Shopping Carts " Calculate totals, add/store favorites, order status and details ".</li>
          <li>Review system " by Stars and comments, and calculate average sastification ".</li>
        </ul>
        <br />
        <p>Watch Video &darr;</p>
        <br />
        <div className="flex flex-row gap-4 justify-center items-center">

          <iframe
            src="https://www.linkedin.com/embed/feed/update/urn:li:ugcPost:7193920374693052417?compact=1"
            height="399"
            width="504"
            frameBorder="0"
            allowFullScreen
            title="Embedded post"
          ></iframe>

        </div>
        <br />
        <h3 className="text-3xl">Technology Stack</h3>
        <br />
        <ul>
          <li>Frontend: React.js
            <br />
            " Axios, TailwindCSS, Redux/Toolkit for state management".</li>
          <br />
          <li>Backend: Node.js
            <br />
            " Express.js, jsonwebtoken, mongoose, multer ".</li>
          <br />
          <li>Database " MongoDB ".</li>
        </ul>
      </div>
    </>,



    // third
    <>
      <div className="text-xl">

        MERN Stack project " Blog system "
        <br />
        Full Functionality Blogging System with Admin Dashboard,
        <br />
        that allows to create and control posts/articles by category with comment system.
        <br />
        <br />
        <h3 className="text-3xl">Features</h3>
        <br />
        <ul>
          <li>Authentication " Login, Logout, Change Password, User Profile ".</li>
          <li>Autherization " Admin/Admin Dashboard, User/User Profile ".</li>
          <li>Post view & Pagination " Create, Edit, Delete, Homepage view depends on newest posts, and single post page for full details ".</li>
          <li>Admin Dashboard "  Users Control, Posts Control, View control ".</li>
          <li>Comment/Emoji Reactions System " Comment and Reply Functionality, Emoji Reactions ".</li>
          <li>Email Verification " with JWT Technique and Refresh tokens ".</li>
          <li>Responsive Design " Responsive Design via different screens ".</li>
          <li>Cloudinary Third Party " Storing website media posts media / user profile media ".</li>
        </ul>
        <br />
        <p>Watch Videos &darr;</p>
        <br />
        <div className="flex flex-col gap-4 justify-center items-center">

          <iframe
            src="https://www.linkedin.com/embed/feed/update/urn:li:ugcPost:7187781906229280769?compact=1"
            height="399"
            width="504"
            frameBorder="0"
            allowFullScreen
            title="Embedded post"
          ></iframe>



          <iframe
            src="https://www.linkedin.com/embed/feed/update/urn:li:ugcPost:7187783260096131072?compact=1"
            height="399"
            width="504"
            frameBorder="0"
            allowFullScreen
            title="Embedded post"
          ></iframe>



          <iframe
            src="https://www.linkedin.com/embed/feed/update/urn:li:ugcPost:7187785484025155584?compact=1"
            height="399"
            width="504"
            frameBorder="0"
            allowFullScreen
            title="Embedded post"
          ></iframe>



          <iframe
            src="https://www.linkedin.com/embed/feed/update/urn:li:ugcPost:7187786248000856064?compact=1"
            height="399"
            width="504"
            frameBorder="0"
            allowFullScreen
            title="Embedded post"
          ></iframe>



        </div>
        <br />
        <h3 className="text-3xl">Technology Stack</h3>
        <br />
        <ul>
          <li>Frontend: React.js
            <br />
            "Redux for state management, TailwindCSS, axios, react-router-dom ".</li>
          <br />
          <li>Backend: Node.js
            <br />
            " Express.js, cors, jsonwebtoken, mongoose, cloudinary, nodemailer ".</li>
          <br />
          <li>Database " MongoDB ".</li>
        </ul>
      </div>
    </>,
  ];


  return (
    <div className="overflow-x-hidden md:overflow-x-auto">
      <div className='sticky-h3 w-fit text-xs' id="hello1">

        <SectionTitle title="Web-1" progressBgClass="bg-primary"></SectionTitle>

      </div>
      <div className={`cont md:w-[100%] md:-ml-0 w-[170%] -ml-[35%] ${containerActive ? "" : "s--inactive"} ${activeIndex !== null ? "s--el-active" : ""}`}>
        <div className="cont__inner">
          {sections.map((section, index) => (
            <div
              key={index}
              className={`el ${activeIndex === index ? "s--active" : ""}`}
              onClick={() => handleClick(index)}
            >
              <div className="el__overflow">
                <div className="el__inner">
                  <div className="el__bg"></div>
                  <div className="el__preview-cont">
                    <h2 className="el__heading bg-sky-950 w-full text-center">{section}</h2>
                  </div>
                  <div className="el__content overflow-y-scroll overflow-x-hidden mx-auto">
                    <div className="el__text px-[15%]">Details</div>
                    {/* <img src="data:image/jpeg;base64," alt="" />
                    <img src="data:image/jpeg;base64," alt="" />
                    <img src="data:image/jpeg;base64," alt="" />
                    <img src="data:image/jpeg;base64," alt="" /> */}
                    <div className="text-center mx-auto bg-sky-950 opacity-90 p-5">
                      <h4 className="text-3xl ">Overview</h4>
                      <br />
                      {content[index]}
                    </div>
                    <br />


                    {/* <img src={media[index]} alt="" /> */}


                    <div className="el__close-btn mr-[15%]" onClick={handleClose}></div>

                  </div>
                </div>
              </div>
              <div className="el__index">
                <div className="el__index-back">{index + 1}</div>
                <div className="el__index-front">
                  <div className="el__index-overlay" data-index={index + 1}>{index + 1}</div>
                </div>
              </div>
            </div>
          ))}
        </div>
      </div>
    </div>
  );
};

export default InteractiveWeb;
